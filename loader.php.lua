...
if (game.PlaceId ~= 443406476) then
    error(string.format("[SK22T] - Unsupported game | game khong duoc ho tro"));
end

if not (listfiles or hookmetamethod or getcustomasset) then
    Library:Notify("[FATAL-ERROR] - Your exploit doesn't support required functions | executor cua ban khong dap ung duoc tai nguyen can thiet cua chung toi", math.huge);
    return;
end
...
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/vedsyn/fluent/main/source.lua"))()
local Window = Fluent:CreateWindow({
    Title = "Shinichi hub | Attack On Titan Revolution",
...
local Tabs = {
    ...
    Visuals = Window:AddTab({ Title = "Visual" }),
    Misc = Window:AddTab({ Title = "Misc" }),
    Menu = Window:AddTab({ Title = "Settings" })
}
...
Fluent:Notify({
    Title = "Welcome",
    Content = "Shinichi hub loaded",
    SubContent = "sown just too sigma",
    Duration = 5
})

-- Aimbot Tab - Autofarm
local Autofarm = Tabs.Aimbot:AddSection("Autofarm")
...
Autofarm:AddSlider("MaxDistance", {
    Text = "Max Distance",
    Min = 100,
    Max = 1000,
    Default = 500,
    Callback = function(Value)
        Sk22t.Aimbot.Autobot.MaxDistance = Value
    end
})

Autofarm:AddSlider("TweenSpeed", {
    Text = "Tween Speed",
    Min = 1,
    Max = 5,
    Default = 3,
...
Autofarm:AddSlider("MaxMouseDistance", {
...
-- Aimbot Tab - Silent Aim
local Silent = Tabs.Aimbot:AddSection("Silent Aim")
...
-- Visual Tab - Viewmodel
local Viewmodel = Tabs.Visuals:AddSection("Viewmodel")
...
Viewmodel:AddDropdown("Material", {
...
-- Misc Tab - Exploits
local Exploits = Tabs.Misc:AddSection("Exploits")
...
Exploits:AddSlider("PointAmount", {
    Text = "Point Amount",
    Min = 100,
    Max = 10000,
    Default = 1000,
...
-- Menu Tab
local Menu = Tabs.Menu:AddSection("Menu")

Menu:AddButton("Unload", function()
    Fluent:Unload()
end)

Menu:AddKeybind("MenuKeybind", {
    Default = Enum.KeyCode.LeftControl,
    Text = "Menu Keybind",
    Callback = function(Value)
        Fluent.ToggleKeybind = Value
    end
})
